# HurricaneDD的Github Page

## 我的项目

### [安卓APP - RTL SDR LBJ列车预警信号接收和解析器](https://hurricanedd.github.io/Android_RTL_SDR_LBJ_RECEIVER)

安卓端 RTL_SDR_LBJ（列车报警系统）信号接收和解析APP，参考原项目Python脚本完成：https://github.com/Sdr-Is-Fun/RTL_SDR_LBJ_RECEIVER

### [风光摄影模拟器](https://hurricanedd.github.io/scape-sim/)

一个基于CesiumJS的工具，用于模拟在地球上实景的风光摄影。以便于摄影者提前了解风景、太阳光线情况、估算所需焦段。

### [常用气象网站合集](https://hurricanedd.github.io/weather-toolkit/)

个人常用提供天气、气象相关信息网站的导航与汇总。按照适用季节和场景分出“冬季”、“夏季”、“摄影”三个模块。

### [C语言学习笔记-思维导图](https://hurricanedd.github.io/nuaa-c-notes/)

南京航空航天大学 - 程序设计语言（C语言）（经管相关专业）：我的C语言学习笔记 & 思维导图，可供学习参考，欢迎下载。

### [安卓APP - 分类相册](https://hurricanedd.github.io/CategoricalGallery)

与系统相册独立的一个照片管理器，支持将若干相册分配到不同工作区，便于管理照片和相册。

## 联系我

- **Github**：[HurricaneDD](https://github.com/HurricaneDD)
- **QQ**：1727364668
- **B站**：[HurricaneDD](https://b23.tv/qDiePHS)
